...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `gcapidb`.`client_report` (
  `id` CHAR(32) NOT NULL,
  `title` VARCHAR(96) NOT NULL,
...
CREATE UNIQUE INDEX `id_UNIQUE` ON `gcapidb`.`client_report` (`id` ASC) VISIBLE;

CREATE UNIQUE INDEX `title_UNIQUE` ON `gcapidb`.`client_report` (`title` ASC) VISIBLE;


-- -----------------------------------------------------
-- Table `gcapidb`.`client_report_note`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `gcapidb`.`client_report_note` (
  `id` CHAR(32) NOT NULL,
  `client_report_id` CHAR(32) NOT NULL,
  `note_id` CHAR(32) NOT NULL,
  PRIMARY KEY (`client_report_id`, `note_id`),
  CONSTRAINT `client_report_id`
    FOREIGN KEY (`client_report_id`)
    REFERENCES `gcapidb`.`client_report` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `note_id`
    FOREIGN KEY (`note_id`)
    REFERENCES `gcapidb`.`note` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE UNIQUE INDEX `id_UNIQUE` ON `gcapidb`.`client_report_note` (`id` ASC) VISIBLE;

CREATE INDEX `client_report_id_idx1` ON `gcapidb`.`client_report_note` (`client_report_id` ASC) VISIBLE;

CREATE INDEX `note_id_idx` ON `gcapidb`.`client_report_note` (`note_id` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `gcapidb`.`client_website` (
  ...
  PRIMARY KEY (`client_id`, `website_id`),
  CONSTRAINT `client_id`
    FOREIGN KEY (`client_id`)
    REFERENCES `gcapidb`.`client` (`id`)
    ON DELETE NO ACTION
...
CREATE UNIQUE INDEX `filename_UNIQUE` ON `gcapidb`.`file_asset` (`filename` ASC) VISIBLE;
...
CREATE INDEX `file_asset_id_idx` ON `gcapidb`.`gcft_snap` (`file_asset_id` ASC) VISIBLE;
...
CREATE UNIQUE INDEX `address_UNIQUE` ON `gcapidb`.`geocoord` (`address` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `gcapidb`.`ipaddress` (
  ...
  `isp` VARCHAR(255) NULL DEFAULT 'unknown',
  `location` TEXT NULL DEFAULT 'unknown',
  `geocoord_id` CHAR(32) NULL,
  PRIMARY KEY (`address`),
  CONSTRAINT `geocoord_id`
    FOREIGN KEY (`geocoord_id`)
...
CREATE UNIQUE INDEX `auth_id_UNIQUE` ON `gcapidb`.`user` (`auth_id` ASC) VISIBLE;

CREATE UNIQUE INDEX `email_UNIQUE` ON `gcapidb`.`user` (`email` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `gcapidb`.`user_ipaddress` (
  ...
  `user_id` CHAR(32) NOT NULL,
  PRIMARY KEY (`ipaddress_id`, `user_id`),
